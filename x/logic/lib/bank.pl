...
% validate_bech32_address(+Address) is det.
%
% Verifies that Address is valid Bech32 and maps only this specific validation error
% to bank_balances/2 while leaving all other errors untouched.
validate_bech32_address(Address) :-
  catch(
    bech32_address(_, Address),
    Error,
    normalize_bech32_error(Error, Address)
  ).

normalize_bech32_error(Error, Address) :-
  (   bech32_domain_error(Error)
  ->  throw(error(domain_error(encoding(bech32), Address), bank_balances/2))
  ;   throw(Error)
  ).
