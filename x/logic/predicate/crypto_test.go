...
ed25519_verify(PubKey, Msg, Sig, encoding(octet)).`,
				query:       `verify.`,
				wantResult:  []types.TermResults{{}},
				wantSuccess: true,
			},
			...
			ed25519_verify(PubKey, Msg, Sig, encoding(octet)).`,
				query:       `verify.`,
				wantSuccess: false,
				wantError:   fmt.Errorf("ed25519_verify/4: failed verify signature: ed25519: bad public key length: 33"),
			},
			{ // Wrong signature
				program: `verify :-
			hex_bytes('53167ac3fc4b720daa45b04fc73fe752578fa23a10048422d6904b7f4f7bba5a', PubKey),
			...
			ecdsa_verify(PubKey, Msg, Sig, encoding(octet)).`,
				query:       `verify.`,
				wantSuccess: false,
				wantError:   fmt.Errorf("ecdsa_verify/4: failed verify signature: failed decode PEM public key"),
			},
			{
				// Wrong msg
				program: `verify :-
			...
			Convey(fmt.Sprintf("Given the query #%d: %s", nc, tc.query), func() {
				Convey("and a context", func() {
					...
					Convey("and a vm", func() {
						interpreter := testutil.NewLightInterpreterMust(ctx)
						interpreter.Register2(engine.NewAtom("hex_bytes"), HexBytes)
						interpreter.Register4(engine.NewAtom("ed25519_verify"), ED25519Verify)
