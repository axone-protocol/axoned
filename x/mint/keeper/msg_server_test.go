...
func TestUpdateParams(t *testing.T) {
	Convey("Given a mint msg server", t, func() {
		testCtx := setupIntegrationTestContext(t)
		...
		testCases := []struct {
			...
			},
			{
				...
				request: &types.MsgUpdateParams{
					Authority: testCtx.mintKeeper.GetAuthority(),
					Params: types.Params{
		...
		for _, tc := range testCases {
			Convey(tc.name, func() {
				_, err := testCtx.msgServer.UpdateParams(testCtx.ctx, tc.request)
				if tc.expectErr {
