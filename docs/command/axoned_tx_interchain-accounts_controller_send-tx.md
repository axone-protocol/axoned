## axoned tx interchain-accounts controller send-tx

Send an interchain account tx on the provided connection.

### Synopsis

Submits pre-built packet data containing messages to be executed on the host chain and attempts to send the packet.
...
```
axoned tx interchain-accounts controller send-tx [connection-id] [path/to/packet_msg.json] [flags]
```
...
```
  ...
  -o, --output string                   Output format (text|json) (default "json")
      --packet-timeout-timestamp uint   Packet timeout timestamp in nanoseconds from now. Default is 10 minutes. (default 600000000000)
